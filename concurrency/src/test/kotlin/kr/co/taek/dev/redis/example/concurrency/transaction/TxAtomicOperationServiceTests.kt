...
@ActiveProfiles("test")
@SpringBootTest
class TxAtomicOperationServiceTests {
    @Autowired
    private lateinit var txAtomicOperationService: TxAtomicOperationService
    ...
    fun increment_with_redis_transaction() {
        ...
        repeat(2) {
            val future = CompletableFuture.runAsync { txAtomicOperationService.increment(key, true) }
            futures.add(future)
        }
